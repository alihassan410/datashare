...
import { getProductById, updateProduct } from "../../../api/internal";
...
const EditProduct = (props) => {
    ...
    const { values, touched, handleBlur, handleChange, errors, handleSubmit, setValues } = formik;

    useEffect(() => {
        const getProductClient = async () => {
            let res = await getProductById(props.id);
    ...
    useEffect(() => {
        if (newProduct) {
            setValues({
                ...
                expiry_date: newProduct.expiry_date ? newProduct.expiry_date.split("T")[0] : "",
                _id: newProduct._id,
            });
        }
    }, [newProduct, setValues]);
